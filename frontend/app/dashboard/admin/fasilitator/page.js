    ...
    const fetchStats = async () => {
      try {
        const [schoolsRes, studentsRes] = await Promise.all([
          fetchInstance('/api/admin/overview/active-schools'),
          fetchInstance('/api/admin/overview/total-students')
        ]);
        if (schoolsRes?.success) setActiveSchools(schoolsRes.activeSchools);
        if (studentsRes?.success) setTotalStudents(studentsRes.totalStudents);
      } catch (error) {
  ...
  return (
    <>
      <style dangerouslySetInnerHTML={{__html: `
        ...
        .fasilitator-wrapper .prog-current{font-size:24px;font-weight:800;color:#0f172a;line-height:1}
        .fasilitator-wrapper .prog-target{font-size:12px;color:#94a3b8;font-weight:600}
        .fasilitator-wrapper .prog-bar-wrap{background:#e2e8f0;height:6px;border-radius:3px;margin-bottom:6px;overflow:hidden}
        .fasilitator-wrapper .prog-bar{background:#0D9488;height:100%;border-radius:3px;transition:width .6s}
        .fasilitator-wrapper .prog-pct{font-size:11px;color:#0D9488;font-weight:700;text-align:right}
        .fasilitator-wrapper .stats-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:16px}
        .fasilitator-wrapper .stat-card{background:#ffffff;border:1px solid #e2e8f0;border-radius:12px;padding:16px;box-shadow:0 1px 2px rgba(0,0,0,0.05)}
        .fasilitator-wrapper .stat-lbl{font-size:10px;color:#64748b;font-weight:700;text-transform:uppercase;margin-bottom:4px}
        .fasilitator-wrapper .stat-val{font-size:20px;font-weight:800;color:#0f172a}
        .fasilitator-wrapper .map-section{background:#ffffff;border:1px solid #e2e8f0;border-radius:12px;padding:18px;box-shadow:0 1px 2px rgba(0,0,0,0.05)}
        .fasilitator-wrapper .cities-grid{display:grid;grid-template-columns:repeat(5,1fr);gap:10px;margin-top:14px}
        .fasilitator-wrapper .city-btn{background:#f8fafc;border:1px solid #e2e8f0;padding:10px;border-radius:8px;text-align:center;cursor:pointer;transition:all .2s;color:#64748b}
        .fasilitator-wrapper .city-btn:hover{background:#f1f5f9;border-color:#cbd5e1;color:#0f172a}
        .fasilitator-wrapper .city-btn.active{background:#0D9488;border-color:#0D9488;color:#fff}
        .fasilitator-wrapper .city-btn.active .city-metric-lbl{color:rgba(255,255,255,0.8)}
        .fasilitator-wrapper .city-btn.active .city-metric-val{color:#fff}
        .fasilitator-wrapper .city-btn.active .city-mini-bar{background:rgba(255,255,255,0.2)}
        .fasilitator-wrapper .city-btn.active .city-mini-bar-fill{background:#fff}
        .fasilitator-wrapper .city-name{font-size:11px;font-weight:700;text-transform:uppercase;margin-bottom:8px;letter-spacing:.05em}
        .fasilitator-wrapper .city-metrics{display:grid;grid-template-columns:1fr 1fr;gap:4px;text-align:left}
        .fasilitator-wrapper .city-metric{border-right:1px solid #e2e8f0;padding-right:4px}
        .fasilitator-wrapper .city-metric:last-child{border:none}
        .fasilitator-wrapper .city-metric-lbl{font-size:10px;color:#94a3b8}
        .fasilitator-wrapper .city-metric-val{font-size:12px;font-weight:700;color:#0f172a}
        .fasilitator-wrapper .city-metric-val.teal{color:#0D9488}
        .fasilitator-wrapper .city-metric-val.purple{color:#7C3AED}
        .fasilitator-wrapper .city-mini-bar{background:#e2e8f0;border-radius:3px;height:4px;margin-top:8px;overflow:hidden}
        ...
        .fasilitator-wrapper .panel{background:#ffffff;border:1px solid #e2e8f0;border-radius:12px;padding:16px 18px;box-shadow:0 1px 2px rgba(0,0,0,0.05)}
        .fasilitator-wrapper .panel-title{font-size:12px;font-weight:700;color:#64748b;margin-bottom:14px;display:flex;align-items:center;gap:6px}
        ...
        .fasilitator-wrapper .phase-label{font-size:11px;color:#64748b;width:80px;flex-shrink:0}
        .fasilitator-wrapper .phase-bar-wrap{flex:1;background:#e2e8f0;border-radius:4px;height:14px;overflow:hidden;position:relative}
        ...
        .fasilitator-wrapper .phase-val{font-size:11px;color:#0f172a;width:32px;text-align:right;font-weight:600}
        .fasilitator-wrapper .tool-row{display:flex;align-items:center;gap:8px;margin-bottom:8px}
        .fasilitator-wrapper .tool-name{font-size:11px;color:#64748b;width:70px;flex-shrink:0}
        .fasilitator-wrapper .tool-bar-wrap{flex:1;background:#e2e8f0;border-radius:4px;height:10px;overflow:hidden}
        .fasilitator-wrapper .tool-bar-fill{height:10px;border-radius:4px}
        .fasilitator-wrapper .tool-pct{font-size:11px;color:#64748b;width:30px;text-align:right;font-weight:600}
        .fasilitator-wrapper .feed-item{display:flex;gap:10px;padding:8px 0;border-bottom:1px solid #e2e8f0}
        ...
        .fasilitator-wrapper .feed-title{font-size:12px;color:#0f172a;margin-bottom:2px;font-weight:600}
        .fasilitator-wrapper .feed-meta{font-size:10px;color:#94a3b8}
        .fasilitator-wrapper .sdg-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:8px}
        .fasilitator-wrapper .sdg-card{border-radius:8px;padding:10px 12px;text-align:center;background:#f8fafc;border:1px solid #e2e8f0}
        .fasilitator-wrapper .sdg-num{font-size:18px;font-weight:700;margin-bottom:2px;color:#0f172a}
        .fasilitator-wrapper .sdg-lbl{font-size:9px;font-weight:600;letter-spacing:.06em;color:#64748b}
        .fasilitator-wrapper .sdg-desc{font-size:10px;margin-top:4px;line-height:1.3;opacity:.85;color:#475569}
        .fasilitator-wrapper .timestamp{font-size:10px;color:#94a3b8;text-align:right;padding-top:4px}
      `}} />
          ...
          <div>
            ...
            <div className="progress-row">
              <div className="prog-card">
                <div className="prog-title"><i className="ti ti-building" aria-hidden="true"></i>Target Sekolah</div>
                <div className="prog-nums"><span className="prog-current">{activeSchools}</span><span className="prog-target">/ 100 Sekolah</span></div>
                <div className="prog-bar-wrap"><div className="prog-bar" style={{width: `${Math.min(100, (activeSchools/100)*100)}%`}}></div></div>
                <div className="prog-pct">{Math.round((activeSchools/100)*100)}% tercapai</div>
              </div>
              <div className="prog-card">
                <div className="prog-title"><i className="ti ti-school" aria-hidden="true"></i>Target Siswa</div>
                <div className="prog-nums"><span className="prog-current">{totalStudents.toLocaleString('id-ID')}</span><span className="prog-target">/ 1.000 Siswa</span></div>
                <div className="prog-bar-wrap"><div className="prog-bar" style={{width: `${Math.min(100, (totalStudents/1000)*100)}%`, background: "#7C3AED"}}></div></div>
                <div className="prog-pct" style={{color: "#7C3AED"}}>{Math.round((totalStudents/1000)*100)}% tercapai</div>
              </div>
              <div className="prog-card">
