...
import { 
  ...
  UserCheck, UserX, Filter, Eye, Calendar, 
          ...
          <div className="bg-white w-full max-w-lg rounded-[40px] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 border border-slate-100">
            ...
            <div className="p-8 space-y-6">
              <div className="grid grid-cols-2 gap-4">
                ...
                <DetailItem label="Bergabung Pada" value={new Date(selectedUser.createdAt).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })} icon={<Calendar size={14}/>} />
                <DetailItem 
                  label="Instansi" 
            ...
            <div className="p-8 bg-slate-50/50 flex gap-3">
              <button className="flex-1 py-4 bg-[#00adb5] text-white rounded-2xl font-black text-[11px] uppercase tracking-widest shadow-lg shadow-[#00adb5]/20">
                Kirim Notifikasi
              </button>
      ...
      <div className="flex flex-col lg:flex-row justify-between items-center gap-6">
        ...
        <div className="flex flex-col md:flex-row gap-4 w-full lg:w-auto">
          ...
          <div className="bg-white px-4 py-2.5 rounded-2xl border border-slate-200 flex items-center gap-3 shadow-sm">
            ...
            <select 
              className="bg-transparent outline-none text-xs font-bold text-slate-600"
              value={roleFilter}
            ...
            <tbody className="divide-y divide-slate-200">
              {loading ? (
                ...
                <tr key={user._id} className="hover:bg-slate-50/50 transition-all">
                  <td className="px-6 py-6 border border-slate-200 text-center text-xs font-black text-slate-300">{index + 1}</td>
