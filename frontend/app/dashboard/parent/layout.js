    ...
    <div className="flex min-h-screen bg-[#fafbfc] font-sans">
      ...
      <aside className="hidden lg:flex w-72 bg-white border-r border-slate-100 flex-col sticky top-0 h-screen shadow-sm">
        ...
        <div className="p-6 border-t border-slate-50 space-y-1">
          <ParentNavItem href="/dashboard/parent/settings" icon={<Settings size={20} />} label="Profil Ayah/Bunda" />
      ...
      <main className="flex-1 flex flex-col min-w-0">
        ...
        <header className="h-20 bg-white border-b border-slate-100 px-10 flex items-center justify-between sticky top-0 z-20">
          <div className="flex items-center bg-slate-50 px-5 py-2.5 rounded-2xl w-full max-w-md border border-slate-100">
            ...
            <input type="text" placeholder="Cari laporan atau artikel..." className="bg-transparent outline-none text-sm w-full font-medium" />
          </div>
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="flex items-center gap-3 pl-6 border-l border-slate-100">
              <div className="text-right hidden sm:block">
                <p className="text-sm font-bold text-slate-800 leading-none">Ayah Muhammad</p>
                <p className="text-[10px] font-bold text-primary mt-1 uppercase tracking-tight">Akun Orang Tua</p>
              </div>
              <div className="w-11 h-11 rounded-2xl bg-secondary flex items-center justify-center font-black text-slate-800 shadow-lg shadow-secondary/20">
                M
              </div>
            </div>
          </div>
...
/**
 * Komponen NavItem khusus Parent dengan styling yang lebih lembut
 */
function ParentNavItem({ href = "#", icon, label, active = false, danger = false }) {
  return (
    <Link href={href} className={`
      flex items-center gap-4 px-5 py-4 rounded-2xl cursor-pointer transition-all duration-300 font-bold text-sm
