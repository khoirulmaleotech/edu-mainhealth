...
import { Search, Loader2, HeartPulse, Download } from "lucide-react";
...
export default function TeacherTilikDiriPage() {
  ...
  const fetchData = async (params = {}) => {
    try {
      ...
      const res = await fetchInstance(`/api/teacher/tilik-diri?${queryParams.toString()}`);
      if (res.success) {
        setData(res.data);
  ...
  const handleExportExcel = async () => {
    try {
      setLoading(true);
      const res = await fetchInstance(`/api/teacher/tilik-diri?export=true&search=${encodeURIComponent(search)}`);
      ...
      if (res.success && res.data) {
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, "Hasil Tilik Diri Siswa");
        
        // Generate dan download file excel
        XLSX.writeFile(workbook, `Tilik_Diri_Siswa_${new Date().toISOString().split('T')[0]}.xlsx`);
      }
    } catch (error) {
    ...
    <div className="font-sans">
      <div className="mb-8">
        ...
        <p className="text-slate-400 font-medium mt-2">
          Pantau hasil asesmen Tilik Diri dari semua siswa di sekolah Anda.
        </p>
      </div>
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan="6" className="py-20 text-center">
                    ...
                    <p className="text-slate-400 text-sm font-medium">Belum ada data Tilik Diri dari siswa di sekolah Anda.</p>
                  </td>
                </tr>
              ) : (
                data.map((item, index) => (
